...
def subir_imagen_grupo_model(imagen_path):
    try:
        with open(imagen_path, "rb") as f:
            path = f"grupos/{session['id_usuario']}/{str(datetime.now()).replace(' ', '_').replace(':', '-')}_{imagen_path.split('/')[-1]}"
            image_response = supabase_client.storage \
                .from_("SocialMindMedia") \
                .upload(
                    file=f,
        ...
        print(f"Error al subir la imagen del grupo: {e}")
        return {"status": False, "error": str(e), "path": None}
    
def subir_banner_grupo_model(imagen_path):
    try:
        with open(imagen_path, "rb") as f:
            path = f"banner_grupos/{session['id_usuario']}/{str(datetime.now()).replace(' ', '_').replace(':', '-')}_{imagen_path.split('/')[-1]}"
            image_response = supabase_client.storage \
                .from_("SocialMindMedia") \
                .upload(
                    file=f,
